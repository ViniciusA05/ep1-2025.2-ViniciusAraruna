...
public class RelatoriosServico {
    ...
    public RelatoriosServico(PacienteServico ps, MedicoServico ms, ConsultaServico cs, InternacaoServico is, PlanoSaudeServico pss){
        this.pacienteServico = ps;
        this.consultaServico = cs;
        this.internacaoServico = is;
        this.medicoServico = ms;
        this.planoSaudeServico = pss;
    }
...
//historico de consultas
    ...
    public void relatorioEstatisticoPlanoSaude() {
        System.out.println("\n--- ESTATÍSTICA: ECONOMIA POR PLANO DE SAÚDE ---");

        Map<String, Double[]> estatisticas = new HashMap<>();
        for (PlanoSaude plano : planoSaudeServico.listarPlanos()) {
            estatisticas.put(plano.getNomePlano(), new Double[]{0.0, 0.0});
        }
        ...
        for (Consulta c : consultaServico.listaConsultas()) {
            ...
            if (c.getPaciente() instanceof PacienteEspecial) {
                ...
                double economiaDessaConsulta = custoBaseMedico - custoFinalPago;

                if (estatisticas.containsKey(nomePlano)) {
                    Double[] dadosAtuais = estatisticas.get(nomePlano);
                    dadosAtuais[0]++;
                    dadosAtuais[1] += economiaDessaConsulta;
                }
            }
        }
        ...
        if (estatisticas.isEmpty()) {
            System.out.println("Nenhum Plano de Saúde cadastrado para análise.");
        } else {
            for (Map.Entry<String, Double[]> entry : estatisticas.entrySet()) {
                Double[] dados = entry.getValue();
                System.out.printf("Plano: %s\n", entry.getKey());
                System.out.printf("  > Total de Consultas/Usos: %.0f\n", dados[0]);
                System.out.printf("  > Economia Total para Usuários: R$ %.2f\n", dados[1]);
                System.out.println("-------------------------------------");
            }
        }
    }
